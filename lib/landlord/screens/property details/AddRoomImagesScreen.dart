...
import '../../providers/AddRoomImagesProvider.dart';

class AddRoomImagesScreen extends StatefulWidget {
  final String propertyId;
  final String roomId;

  const AddRoomImagesScreen({
    super.key,
  ...
  @override
  State<AddRoomImagesScreen> createState() => _AddRoomImagesScreenState();
}
...
class _AddRoomImagesScreenState extends State<AddRoomImagesScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AddRoomImageProvider>().reset();
    });
  }
  ...
  Future<void> _pickImages() async {
    try {
      ...
      if (images.isNotEmpty && mounted) {
        final provider = context.read<AddRoomImageProvider>();
        final currentImages = provider.selectedImages;
        final updatedImages = [...currentImages, ...images];

        // Limit to maximum 10 images
        if (updatedImages.length > 10) {
          final limitedImages = updatedImages.take(10).toList();
          provider.updateSelectedImages(limitedImages);
          ...
          provider.updateSelectedImages(updatedImages);
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _pickImageFromCamera() async {
    try {
      ...
      if (image != null && mounted) {
        ...
        provider.addSelectedImage(image);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _removeImage(int index) {
    context.read<AddRoomImageProvider>().removeSelectedImage(index);
  }
  ...
  Future<void> _uploadImages() async {
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Room images uploaded successfully!'),
            backgroundColor: Colors.green,
          ),
        );

        // Pop both screens and return true to indicate success
        Navigator.of(context).pop(true); // Pop AddRoomImagesScreen
        Navigator.of(context).pop(true); // Pop AddRoomScreen
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<AddRoomImageProvider>(
      builder: (context, provider, child) {
        ...
        return Scaffold(
          ...
          appBar: _buildAppBar(provider.selectedImages),
          body: FadeTransition(
            opacity: _fadeAnimation,
            child: Column(
              children: [
                Expanded(child: _buildContent(provider.selectedImages)),
                _buildBottomActions(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Add up to 10 high-quality images of your room. First image will be the cover photo.',
              style: TextStyle(
                fontSize: AppSizes.smallText(context),
                color: AppColors.primary,
              ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: AppSizes.smallPadding(context)),
            Text(
              'Add attractive photos of your room to get more inquiries',
  ...
  Widget _buildBottomActions(List<XFile> selectedImages, bool isLoading) {
    return Container(
      ...
      child: SafeArea(
        child: Row(
          children: [
            Expanded(child: _buildUploadButton(selectedImages, isLoading)),
          ],
        ),
      ),
    );
