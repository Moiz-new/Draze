  ...
  Future<void> addProperty(Map<String, dynamic> propertyData, List<File> imageFiles) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // For error status codes, use the message from JSON if available
        _errorMessage = jsonResponse?['message'] ?? 'Failed to add property. Status: ${response.statusCode}';
        
        // Specific handling if needed, but let the screen decide based on message content
        if (response.statusCode == 401) {
          _errorMessage = jsonResponse?['message'] ?? 'Session expired. Please login again.';
        } else if (response.statusCode == 400) {
          _errorMessage = jsonResponse?['message'] ?? 'Invalid data. Please check all fields.';
        } else if (response.statusCode == 500) {
          _errorMessage = jsonResponse?['message'] ?? 'Server error. Please try again later.';
        }
      }
      notifyListeners();
    } catch (e) {
