...
import '../../providers/SellerAllVisitorsProvider.dart';
import 'SellerVisitorDetailsScreen.dart';

class SellerAllVisitorsScreen extends StatefulWidget {
  const SellerAllVisitorsScreen({Key? key}) : super(key: key);

  @override
  State<SellerAllVisitorsScreen> createState() =>
      _SellerAllVisitorsScreenState();
}

class _SellerAllVisitorsScreenState extends State<SellerAllVisitorsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<SellerAllVisitorsProvider>().fetchAllVisitors();
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<SellerAllVisitorsProvider>(
  ...
  Widget _buildModernAppBar(SellerAllVisitorsProvider provider) {
    return SliverAppBar(
  ...
  Widget _buildFilterSection(SellerAllVisitorsProvider provider) {
  ...
  Widget _buildVisitorsList(SellerAllVisitorsProvider provider) {
                ...
                if (mounted) {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder:
                          (context) =>
                              SellerVisitorDetailsScreen(visitor: visitor),
                    ),
                  );
                }
              },
  ...
  Widget _buildErrorState(SellerAllVisitorsProvider provider) {
    return Container(
...
class ModernVisitorCard extends StatelessWidget {
  ...
  String _getLandlordName() {
    return visitor.name ?? 'Unknown Landlord';
  }

  String _getLandlordMobile() {
    return visitor.mobile ?? 'No contact';
  }
            ...
            Container(
              ...
              child: Row(
                children: [
                    ...
                    child: _buildQuickAction(
                      ...
                      () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (context) => SellerVisitorDetailsScreen(
                                  visitor: visitor,
                                ),
                          ),
                        );
                  ...
                  // Only show Confirm and Cancel buttons when status is PENDING
                  if (_isPending()) ...[
                    SizedBox(width: 12.w),
                    Expanded(
                      child: _buildQuickAction(
  ...
  bool _isPending() {
    final status = visitor.status.toUpperCase().trim();
    return status == 'PENDING';
  }

  String _formatVisitDate() {
    try {
      return DateFormat('MMM dd, yyyy • hh:mm a').format(visitor.scheduledDate);
    } catch (e) {
                            ...
                            : () async {
                              ...
                              try {
                                final visitorProvider =
                                    Provider.of<SellerAllVisitorsProvider>(
                                      context,
                                      listen: false,
                                    );
